...
class BookDetailScreen extends StatelessWidget {
  ...
  void _openPdf(BuildContext context) {
    if (book.pdfUrl.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('PDF not available yet')));
      return;
    }
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => PdfViewerScreen(title: book.title, pdfUrl: book.pdfUrl)));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text(book.title, style: const TextStyle(fontSize: 15)),
        ...
        actions: [IconButton(icon: const Icon(Icons.share, color: Colors.white),
          onPressed: () => Share.share('${book.title}\n${book.description}\n\n${AppStrings.website}'))]),
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Center(child: Container(
            ...
            child: Center(child: Text('YCT', style: TextStyle(color: Colors.white.withOpacity(0.9), fontSize: 16, fontWeight: FontWeight.bold))),
          )),
          ...
          if (book.titleTelugu.isNotEmpty) ...[
            ...
            Center(child: Text(book.titleTelugu, style: const TextStyle(fontSize: 14, color: AppColors.textMid))),
          ],
          ...
          SizedBox(width: double.infinity,
            child: ElevatedButton.icon(
              ...
              label: const Text('Read this book', style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w600)),
