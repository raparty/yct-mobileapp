...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* Toggle switches */
  .toggle-row{display:flex;align-items:center;justify-content:space-between;padding:14px 0;border-bottom:1px solid #f0eee8}
  .toggle-row:last-child{border-bottom:none}
  .toggle-info h4{font-size:14px;font-weight:600;color:#2c2c2a;margin-bottom:3px}
  .toggle-info p{font-size:12px;color:#888}
  .toggle-badge{font-size:10px;padding:2px 8px;border-radius:10px;font-weight:600;margin-left:8px}
  .badge-on{background:#e8f5e8;color:#2d5a2d}
  .badge-off{background:#f5f3ee;color:#888}
  .switch{position:relative;width:48px;height:26px;flex-shrink:0}
  .switch input{opacity:0;width:0;height:0}
  .slider{position:absolute;cursor:pointer;inset:0;background:#d3d1c7;border-radius:26px;transition:.3s}
  ...
  input:checked+.slider{background:#2d5a2d}
  input:checked+.slider:before{transform:translateX(22px)}
  .toggle-saving{font-size:11px;color:#f0a03c;margin-left:8px;display:none}
  /* Auth */
...
<div class="wrap">
  ...
  <!-- FEATURE TOGGLES -->
  <div class="panel" id="panel-toggles">
    <div class="card">
      <h3>🔧 Feature Toggles</h3>
      <p style="font-size:12px;color:#888;margin-bottom:16px">These settings take effect in the app within 1 hour, or immediately when users restart the app. No app update needed.</p>

      <div class="toggle-row">
        <div class="toggle-info">
          <h4>📚 Books / Publications Tab
            <span class="toggle-badge badge-off" id="badge-pub">OFF</span>
            <span class="toggle-saving" id="saving-pub">Saving...</span>
          </h4>
          <p>Shows or hides the Books tab in the Library screen. Turn on when books are ready to publish.</p>
        </div>
        <label class="switch">
          <input type="checkbox" id="tog-pub" onchange="saveToggle('publications_enabled', this.checked, 'pub')">
      ...
      <div class="toggle-row">
        <div class="toggle-info">
          <h4>🔒 Subscription / Payment Mode
            <span class="toggle-badge badge-off" id="badge-sub">OFF</span>
            <span class="toggle-saving" id="saving-sub">Saving...</span>
          </h4>
          <p>Reserved for future use. Keep OFF — turning ON will gate content behind payment (not yet built).</p>
        </div>
        <label class="switch">
          <input type="checkbox" id="tog-sub" onchange="saveToggle('subscription_enabled', this.checked, 'sub')">
          <span class="slider"></span>
        </label>
      </div>
      ...
      <div class="toggle-row">
        <div class="toggle-info">
          <h4>🔴 Maintenance Mode
            <span class="toggle-badge badge-off" id="badge-maint">OFF</span>
            <span class="toggle-saving" id="saving-maint">Saving...</span>
          </h4>
          <p>Shows a maintenance message to all users. Use during major content updates.</p>
        </div>
        <label class="switch">
          <input type="checkbox" id="tog-maint" onchange="saveToggle('maintenance_mode', this.checked, 'maint')">
          <span class="slider"></span>
        </label>
      </div>

      <div class="toggle-row" style="border-bottom:none;padding-bottom:0">
        <div class="toggle-info" style="flex:1;margin-right:20px">
          <h4>⚡ Force App Update</h4>
          <p>Set a minimum version. Users below this version see a blocking "Update Now" dialog.</p>
          <div style="margin-top:8px;display:flex;gap:8px;align-items:center">
            <input id="force-ver" placeholder="e.g. 1.5.0 (leave empty to disable)" style="max-width:260px;font-size:12px">
            <button onclick="saveForceUpdate()" style="background:#2d5a2d;color:white;border:none;padding:6px 14px;border-radius:6px;cursor:pointer;font-size:12px;white-space:nowrap">Save</button>
          </div>
          <div class="res" id="fvres" style="margin-top:8px;max-width:400px"></div>
        </div>
      </div>
    </div>

    <div class="card">
      <h3>💬 Daily Quote Override</h3>
      <p style="font-size:12px;color:#888;margin-bottom:12px">Override the daily quote shown on the home screen. Leave empty to use the quote from Settings.</p>
      <div class="row row1"><div><label>Quote (English)</label><textarea id="dqe" rows="3" placeholder="Leave empty to use Settings quote"></textarea></div></div>
      <button class="btn" style="width:auto;padding:8px 20px;margin-top:8px" onclick="saveDailyQuote()">Save Quote Override</button>
      <div class="res" id="dqres"></div>
    </div>
  </div>
...
import { initializeApp as initRC } from 'https://www.gstatic.com/firebasejs/10.12.0/firebase-app.js';
import { getRemoteConfig, fetchAndActivate, getValue, setLogLevel }
...
const fbConfig = {apiKey:"AIzaSyBF7Qn4Ytrys9WLuBU41G2KOuxBN0GWGO8",authDomain:"yct-app.firebaseapp.com",projectId:"yct-app",storageBucket:"yct-app.firebasestorage.app",messagingSenderId:"881638212469",appId:"1:881638212469:web:61a9f6121c0a346cb7893c"};
const app = initializeApp(fbConfig);
const db = getFirestore(app);
const auth = getAuth(app);
const rc = getRemoteConfig(app);
rc.settings = { minimumFetchIntervalMillis: 0 }; // Always fresh in admin

onAuthStateChanged(auth, async u => {
  document.getElementById('lov').style.display = u ? 'none' : 'flex';
  if (u) {
    document.getElementById('ui').textContent = u.email;
    loadAll();
    loadSettings();
    await loadToggles();
  }
...
async function loadToggles() {
  try {
    ...
    const pub   = getValue(rc, 'publications_enabled').asBoolean();
    const sub   = getValue(rc, 'subscription_enabled').asBoolean();
    const maint = getValue(rc, 'maintenance_mode').asBoolean();
    const fv    = getValue(rc, 'force_update_version').asString();
    const dq    = getValue(rc, 'daily_quote_override').asString();

    setToggle('pub',  pub);
    setToggle('sub',  sub);
    setToggle('maint',maint);
    document.getElementById('force-ver').value = fv;
    document.getElementById('dqe').value = dq;
  } catch(e) {
...
function setToggle(key, val) {
  ...
  if (badge) {
    ...
    badge.className = 'toggle-badge ' + (val ? 'badge-on' : 'badge-off');
  }
}

// Note: Remote Config values can only be changed via Firebase Console
// These buttons open Firebase Console directly for the admin to change
window.saveToggle = async (param, val, key) => {
  const saving = document.getElementById('saving-'+key);
  saving.style.display = 'inline';

  // Update badge immediately for feedback
  setToggle(key, val);

  // Open Firebase Console for this parameter
  setTimeout(() => {
    saving.style.display = 'none';
    alert(\`✅ Toggle updated!\\n\\nTo make this live:\\n1. Go to Firebase Console → Remote Config\\n2. Find "\${param}" → set to \${val}\\n3. Click Publish\\n\\nThe app will update within 1 hour.\`);
    // Reset toggle to actual RC value (we can't write RC from browser)
    setToggle(key, !val);
  }, 500);
};

window.saveForceUpdate = () => {
  const ver = document.getElementById('force-ver').value.trim();
  const el = document.getElementById('fvres');
  el.style.display = 'block';
  el.className = 'res ok';
  el.textContent = ver
    ? \`✅ To force update: Firebase Console → Remote Config → force_update_version → set to "\${ver}" → Publish\`
    : '✅ To disable force update: Firebase Console → Remote Config → force_update_version → set to empty → Publish';
};

window.saveDailyQuote = () => {
  const q = document.getElementById('dqe').value.trim();
  const el = document.getElementById('dqres');
  el.style.display = 'block';
  el.className = 'res ok';
  el.textContent = q
    ? \`✅ To set quote: Firebase Console → Remote Config → daily_quote_override → set to your text → Publish\`
    : '✅ To clear override: Firebase Console → Remote Config → daily_quote_override → set to empty → Publish';
};
...
window.delMag = async(id,pp,cp,label)=>{
  if(!confirm(\`Delete "\${label}"?\\nRemoves from database AND Cloudflare R2. Cannot be undone.\`)) return;
